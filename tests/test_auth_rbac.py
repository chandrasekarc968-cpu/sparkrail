...
def test_read_only_operator_role_restrictions(client, rbac_client):
    """
    Verifies that READ_ONLY_OPERATOR can view capabilities but is strictly
    forbidden from issuing block sanctions or civil block requests.
    """
    login_res = client.post("/api/v1/auth/login", json={
        "identifier": "reader.ddu@indianrailways.gov.in",
        "password": DEFAULT_PASSWORD
    })
    assert login_res.status_code == 200
    user_data = login_res.json()["user"]
    assert user_data["role"] == "READ_ONLY_OPERATOR"
    assert "VIEW_CORRIDOR" in user_data["capabilities"]
    assert "SANCTION_BLOCK" not in user_data["capabilities"]

    token = login_res.json()["access_token"]

    # Attempting to execute any privileged action returns 403
    res = rbac_client.post("/test/ctpc-isolation", headers={"Authorization": f"Bearer {token}"})
    assert res.status_code == 403

    res_civil = rbac_client.post("/test/civil-block-request", headers={"Authorization": f"Bearer {token}"})
    assert res_civil.status_code == 403


def test_shadow_and_live_mode_rejects_development_bypass(client, monkeypatch):
    """
    CRITICAL REQUIREMENT: In SHADOW or LIVE mode:
    - DEV_ADMIN_TOKEN is rejected with HTTP 401
    - Arbitrary unauthenticated role headers are rejected with HTTP 401
    - Real signed JWT access tokens are accepted
    """
    from src.api.advisory import router as advisory_router
    test_advisory_app = FastAPI()
    test_advisory_app.include_router(advisory_router)
    adv_client = TestClient(test_advisory_app)

    # 1. In SHADOW Mode
    monkeypatch.setenv("SPARKRAIL_MODE", "shadow")
    monkeypatch.setenv("ALLOW_DEV_BYPASS", "false")

    # Attempt DEV_ADMIN_TOKEN -> must fail with 401
    res_dev_token = adv_client.post(
        "/advisory/proposals",
        headers={"Authorization": "Bearer DEV_ADMIN_TOKEN"},
        json={"division_code": "PRYJ", "horizon_hours": 24}
    )
    assert res_dev_token.status_code == 401
    assert "prohibited outside synthetic" in res_dev_token.json()["detail"]

    # Attempt unauthenticated request with arbitrary X-Actor-Role -> must fail with 401
    res_spoof = adv_client.post(
        "/advisory/proposals",
        headers={"X-Actor-Role": "CTPC", "X-Actor-ID": "SPOOF_01"},
        json={"division_code": "PRYJ", "horizon_hours": 24}
    )
    assert res_spoof.status_code == 401
    assert "strictly prohibited" in res_spoof.json()["detail"]

    # Valid signed JWT in shadow mode -> passes
    srdom_login = client.post("/api/v1/auth/login", json={
        "identifier": "srdom.ddu@indianrailways.gov.in",
        "password": DEFAULT_PASSWORD
    })
    valid_jwt = srdom_login.json()["access_token"]
    res_valid = adv_client.post(
        "/advisory/proposals",
        headers={"Authorization": f"Bearer {valid_jwt}"},
        json={"division_code": "PRYJ", "horizon_hours": 24, "role": "SR_DOM"}
    )
    assert res_valid.status_code == 200

    # 2. In LIVE Mode -> Same strict rejection
    monkeypatch.setenv("SPARKRAIL_MODE", "live")
    res_live_dev = adv_client.post(
        "/advisory/proposals",
        headers={"Authorization": "Bearer DEV_ADMIN_TOKEN"},
        json={"division_code": "PRYJ", "horizon_hours": 24}
    )
